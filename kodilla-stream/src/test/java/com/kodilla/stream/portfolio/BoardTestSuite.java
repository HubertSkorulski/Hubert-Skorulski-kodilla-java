...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        double sums = (double)project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(n -> n.getTasks().stream())
                .map(k -> (int)DAYS.between(k.getCreated(),LocalDate.now()))
                .reduce(0,(sum,current) -> sum + current);
        double count = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(n->n.getTasks().stream())
                .count();
        ...
        double avg = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(n -> n.getTasks().stream())
                .mapToLong(k -> DAYS.between(k.getCreated(),LocalDate.now()))
                .average()
                .getAsDouble();
        //Then
        assertEquals(10,sums/count);
        assertEquals(10,avg);
        assertEquals(avg, sums/count);
